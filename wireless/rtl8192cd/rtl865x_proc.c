...
#if defined(RTL_VLAN_DEBUG)
		if (strncmp(cmd_addr, "enable_rx",9) == 0)
		{
			rx_debug = 1;
		}

		if (strncmp(cmd_addr, "disable_rx",10) == 0)
		{
			rx_debug = 0;
		}

		if (strncmp(cmd_addr, "enable_tx",9) == 0)
		{
			tx_debug = 1;
		}
		if (strncmp(cmd_addr, "disable_tx",10) == 0)
		{
			tx_debug = 0;
		}
		if (strncmp(cmd_addr, "enable_vlan",11) == 0)
		{
			vlan_debug = 1;
		}
		if (strncmp(cmd_addr, "disable_vlan",12) == 0)
		{
			vlan_debug = 0;
		}
		if (strncmp(cmd_addr, "enable_br",9) == 0)
		{
			br_debug = 1;
		}
		if (strncmp(cmd_addr, "disable_br",10) == 0)
		{
			br_debug = 0;
		}
#endif

		...
		unsigned long count, void *data)
{
	...
	rtl_add_wlan_vlan_info((struct vlan_info *)&priv->pmib->vlan, dev);
#endif
...
#if defined(CONFIG_RTL_HW_VLAN_WIFI_BRIDGE)
int rtl_update_wlan_vlan_info(struct net_device *dev, unsigned short vid)
{
   struct rtl8192cd_priv *priv;
   #ifdef NETDEV_NO_PRIV
        priv = ((struct rtl8192cd_priv *)netdev_priv(dev))->wlan_priv;
        ...
        priv = (struct rtl8192cd_priv *)dev->priv;
   #endif
	priv->pmib->vlan.vlan_id = vid;
    //panic_printk("%s %d vid=%d priv->pmib->vlan.vlan_id=%d \n", __FUNCTION__, __LINE__, vid, priv->pmib->vlan.vlan_id);
    return 0;
}
#endif
#endif

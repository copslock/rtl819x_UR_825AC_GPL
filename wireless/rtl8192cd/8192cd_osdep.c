...
#if defined (CONFIG_RTL_WIFI_MCAST_REFINE)
#ifndef TX_NO_MUL2UNI
#define TX_NO_MUL2UNI			1
#endif
extern int mlcst2unicst(struct rtl8192cd_priv *priv, struct sk_buff *skb);
int rtl_mcast_tx_refine(struct sk_buff *skb ,struct net_device *dev)
{
	int i;
#ifdef MBSSID
	int j;
	...
	for (i=0; (i<sizeof(wlan_device)/sizeof(struct _device_info_)) ; i++) {
		if (wlan_device[i].priv && netif_running(wlan_device[i].priv->dev)) {
			...
			priv = wlan_device[i].priv;
			if(wlan_device[i].priv->dev==dev)
			{
				
				if (!priv->pshare->rf_ft_var.mc2u_disable) 
				{
					return mlcst2unicst(priv, skb);
				}
				else
				{
					return __rtl8192cd_start_xmit(skb, priv->dev, TX_NO_MUL2UNI);
				}
			}
			...
			if ((OPMODE & WIFI_AP_STATE) && priv->pmib->miscEntry.vap_enable) {
				for (j=0; j<RTL8192CD_NUM_VWLAN; j++) {
					if ((priv->pvap_priv[j]->assoc_num > 0) && IS_DRV_OPEN(priv->pvap_priv[j]) ) 
					{
						...
						if(priv->pvap_priv[j]->dev==dev)
						{
							
							if (!priv->pvap_priv[j]->pshare->rf_ft_var.mc2u_disable) 
							{
								return mlcst2unicst(priv->pvap_priv[j], skb);
							}
							else
							{
								return __rtl8192cd_start_xmit(skb, priv->pvap_priv[j]->dev, TX_NO_MUL2UNI);
							}
						}
					}
				}

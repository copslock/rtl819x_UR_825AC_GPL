...
uint32 rtk_getTagPortmaskByVid(uint16 vid , void *data_p, struct sk_buff *skb); //rtl_nic.c
#else
...
inline int32 rtl8651_rxPktPreprocess(void *pkt, unsigned int *vid)
{
	...
	*vid = m_pkthdr->ph_vlanId;
	...
	if (srcPortNum < RTL8651_CPU_PORT)
	{
		...
		{	
			/* No CPU bit, finished ASIC forwarding(L2 bridging/L3 Routing/L4 nat translation), only dest ext mbr port... */
			/*
		...
		/*to-do:*/
			if(extPortEnabled)
			{
				if ((m_pkthdr->ph_extPortList & PKTHDR_EXTPORTMASK_CPU) == 0)
				{
					...
					/* No CPU bit, No CPU bit, finished ASIC forwarding(L2 bridging/L3 Routing/L4 nat translation) , only dest ext mbr port... */
					/*
						if dest ph_extPortList 0x1 => to dst ext port 1 => to dst port 1+5=6
						if dest ph_extPortList 0x2 => to dst ext port 2 => to dst port 2+5=7
						if dest ph_extPortList 0x4 => to dst ext port 3 => to dst port 3+5=8
...
#endif			
			#if defined(CONFIG_RTL_TR069_WAN) || defined(CONFIG_RTL_IGMP_VLAN_SUPPORT)
			pPkthdr->ph_txCVlanTagAutoAdd = (0x3f) & rtk_getTagPortmaskByVid(pPkthdr->ph_vlanId,output, (struct sk_buff *)skb); 
			#else
